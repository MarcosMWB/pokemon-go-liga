...
exports.onDesafioResultadosWrite = void 0;
...
async function getBloqueioDesafioMs() {
    try {
        const snap = await adminSdk_1.db.collection("variables").doc("global").get();
        if (!snap.exists)
            return minutesToMs(30); // padrão 30min
        const raw = snap.data()?.bloqueio_desafio_minutos;
 ...
 */
function deveProcessarResultado(before, after) {
    if (!after)
        return false; // deletou
    if (after.status !== "confirmado")
        return false;
    ...
    const beforeProcessed = before && (before.processadoEm !== undefined && before.processadoEm !== null);
    const afterProcessed = after && (after.processadoEm !== undefined && after.processadoEm !== null);
    if (afterProcessed)
        return false;
    if (beforeProcessed)
        return false;
...
async function upsertBloqueioDesafio(params) {
    ...
    const ref = adminSdk_1.db.collection("bloqueios_desafio").doc(bloqueioId);
    await ref.set({
...
async function logEventoDesafio(params) {
    ...
    await adminSdk_1.db.collection("admin_eventos").add({
 ...
 */
exports.onDesafioResultadosWrite = (0, firestore_1.onDocumentWritten)({
    document: "desafios_resultados/{resultadoId}",
    region: "southamerica-east1",
}, async (event) => {
    const resultadoId = event.params?.resultadoId ?? "";
    ...
    */
    ...
    const resRef = adminSdk_1.db.collection("desafios_resultados").doc(resultadoId);
    await resRef.set({ processadoEm: adminSdk_1.FieldValue.serverTimestamp() }, { merge: true });
});
