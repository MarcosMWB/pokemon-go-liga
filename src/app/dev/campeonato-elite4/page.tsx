// src/app/elite4/inscricao/page.tsx
"use client";
...
export default function Elite4InscricaoPage() {
    ...
    const unsub = onSnapshot(qP, async (snap) => {
      ...
      const items = await Promise.all(
        snap.docs.map(async (d) => {
          ...
          // nome do ginásio (cacheado) — resolve para string antes
          let gName = ginasiosCache.get(x.ginasio_id);
          if (!gName) {
            const gDoc = await getDoc(doc(db, "ginasios", x.ginasio_id));
            const resolvedG = gDoc.exists()
              ? ((gDoc.data() as any).nome || x.ginasio_id)
              : x.ginasio_id;
            ginasiosCache.set(x.ginasio_id, resolvedG);
            gName = resolvedG;
          }
          ...
          return {
            ...
            ginasio_nome: gName, // string garantido
          } as Participacao;
        })
      );
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      {msg && (
        <div className="p-3 bg-amber-50 border border-amber-200 rounded text-sm">{msg}</div>
      )}

      {Object.keys(gByLiga).length === 0 ? (
        <div className="p-4 bg-white rounded shadow">
          <p className="text-sm text-gray-600">Você não lidera nenhum ginásio no momento.</p>
        </div>
      ) : (
